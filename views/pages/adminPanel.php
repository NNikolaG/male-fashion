...
if (isset($_SESSION['admin'])) :
    ...
    if (isset($_GET['edit'])) {
        ...
        $colosProd = fetchAllWhere('produktboja', 'idProdukt', '=', $_GET['edit']);
        $pColors = [];
        foreach ($colosProd as $cp) {
            array_push($pColors, $cp->idBoja);
        }

        $sizesProd = fetchAllWhere('produktivelicine', 'idProdukt', '=', $_GET['edit']);
        $pSizes = [];
        foreach ($sizesProd as $sp) {
            array_push($pSizes, $sp->idVelicina);
        }

        $tagsProd = fetchAllWhere('produkttag', 'idProdukt', '=', $_GET['edit']);
        $pTags = [];
        foreach ($tagsProd as $tp) {
            array_push($pTags, $tp->idTag);
        }
    }
    ...
    <section class="product spad">
        <div class="container">
            ...
            <div class="row product__filter">
                            ...
                            <tbody>
                                ...
                                foreach ($createErrors as $log) {
                                    if (!empty($log)) {
                                        $logArr = explode(SEPARATOR, $log);
                                        $value = '';
                                        if (in_array('Files are uploaded Successfully.', $logArr) || in_array('Product successfully updated', $logArr)) {
                                            echo '<tr class="table-success">';
                                        } else {
                                            echo '<tr class="table-danger">';
                                        }
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 mix edit-product" style="display: none;">
                    <div class="product__item">
                        <?php
                        if (isset($_GET['edit'])) :
                        ?>
                            <form action="models/editProduct.php" method="POST">
                                <h4 class="mb-3">Info</h4>
                                <div class="row">
                                    <div class="col-lg-6">
                                        <label for="name">Product Name</label>
                                    ...
                                    <div class="col-4 mb-3">
                                        <label for="Brand">Brand</label>
                                        <?php
                                        echo '<input type="text" name="id" value="' . $_GET['edit'] . '" hidden>';

                                        foreach ($brands as $brand) {
                                            if ($prod->idBrend == $brand->idBrend) {
                                                echo "<li><input type='radio' class='brand' name='radioBrend' value='" . $brand->idBrend . "' checked >&nbsp;&nbsp;&nbsp;&nbsp;" . $brand->brend . "</li>";
                                            } else {
                                                echo "<li><input type='radio' class='brand' name='radioBrend' value='" . $brand->idBrend . "'>&nbsp;&nbsp;&nbsp;&nbsp;" . $brand->brend . "</li>";
                                            }
                                        }
                                        ?>
                                        <span class="help" id="catHelp"></span>
                                    ...
                                    <div class="col-4 mb-3">
                                        ...
                                        foreach ($categories as $category) {
                                            if ($prod->idKategorija == $category->idKategorija) {
                                                echo "<li><input type='radio' class='category' name='cat' value='" . $category->idKategorija . "' checked>&nbsp;&nbsp;&nbsp;&nbsp;" . $category->kategorija . "</li>";
                                            } else {
                                                echo "<li><input type='radio' class='category' name='cat' value='" . $category->idKategorija . "'>&nbsp;&nbsp;&nbsp;&nbsp;" . $category->kategorija . "</li>";
                                            }
                                        }
                                        ?>
                                        <span class="help" id="brandHelp"></span>
                                ...
                                <div class="row">
                                    <div class="col-4 mb-3">
                                        ...
                                        <?php
                                        foreach ($tags as $tag) {
                                            if (in_array($tag->idTag, $pTags)) {
                                                echo '<input type="checkbox" name="Tag[]" value="' . $tag->idTag . '" class="size round" checked>
                                            <label for="' . $tag->tag . '">' . $tag->tag . '
                                            </label>
                                            </br>';
                                            } else {
                                                echo '<input type="checkbox" name="Tag[]" value="' . $tag->idTag . '" class="size round">
                                            <label for="' . $tag->tag . '">' . $tag->tag . '
                                            </label>
                                            </br>';
                                            }
                                    ...
                                    <div class="col-4 mb-3">
                                        ...
                                        <?php
                                        foreach ($sizes as $size) {
                                            if (in_array($size->idVelicina, $pSizes)) {
                                                echo '<input type="checkbox" name="Size[]" value="' . $size->idVelicina . '" class="size round" checked>
                                            <label for="' . $size->velicina . '">' . $size->velicina . '
                                            </label>
                                            </br>';
                                            } else {
                                                echo '<input type="checkbox" name="Size[]" value="' . $size->idVelicina . '" class="size round">
                                            <label for="' . $size->velicina . '">' . $size->velicina . '
                                            </label>
                                            </br>';
                                            }
                                    ...
                                    <div class="col-4 mb-3">
                                        ...
                                        foreach ($colors as $color) {
                                            if (in_array($color->idBoja, $pColors)) {
                                                echo '<input type="checkbox" name="Color[]" value="' . $color->idBoja . '" class="size round" checked>
                                            <label for="' . $color->boja . '">' . $color->boja . '
                                            </label>
                                            </br>';
                                            } else {
                                                echo '<input type="checkbox" name="Color[]" value="' . $color->idBoja . '" class="size round">
                                            <label for="' . $color->boja . '">' . $color->boja . '
                                            </label>
                                            </br>';
                                            }
                                ...
                                <input type="submit" name="submit" value="Edit" class='primary-btn mt-4 uploadBtn'>
                            </form>
                        <?php
                        else :
                        ?>
                            <div class="res-text" style="height: 20vh">
                                <h1> Product not selected</h1>
                            </div>
                        <?php
                        endif;
                        ?>
...
?>
    <div class="res-text" style="height: 40vh">
        <h1 style="margin: 250px"> You are not logged in</h1>
    </div>
<?php
endif;
?>